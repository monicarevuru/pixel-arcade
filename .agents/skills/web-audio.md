# Skill: Web Audio API — 8-bit Sound Engine

## Core Concept
All sound is generated procedurally using the Web Audio API.
No audio files. No external libraries. Pure oscillator-based 8-bit sound.
...
export function getAudioContext() {
  ...
  // Must resume after user gesture (browser policy)
  if (ctx.state === 'suspended') ctx.resume();
  return ctx;
}
```

## Playing a Note (Piano / Zen Keys)
```js
export function playNote(frequency, duration = 0.3, volume = 0.3) {
  ...
  osc.type = 'square'; // 8-bit sound — always use 'square' or 'triangle'
...
## Piano Key Frequencies (C3 to B4 — two octaves)
```js
...
## Sound Effects (Asteroid Dodger)
```js
// Dodge sound — rising blip
...
## Important Rules
- Always lazy-init AudioContext (not at module load — needs user gesture)
- Always use `osc.stop()` to prevent memory leaks
- Keep volume between 0.15 and 0.4 — don't blast the user
- Use `square` wave for retro bleeps, `triangle` for softer tones
- Use `sawtooth` for buzz/alarm effects only
